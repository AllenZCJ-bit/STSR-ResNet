...
def create_sliding_windows_with_time_scale(data_sequence, window_size=105, stride=105, time_scale_ratio=1.0):
    """
    使用固定间隔的滑动窗口生成图像，支持时间轴尺度控制

    Parameters:
        data_sequence: 输入数据序列
        window_size: 窗口大小（图像边长）
        stride: 滑动步长
        time_scale_ratio: 时间轴尺度比例 (1.0为原始尺度，>1.0时间轴压缩，<1.0时间轴拉伸)
    """
    print("正在生成滑动窗口图像...")
    print(f"时间轴尺度比例: {time_scale_ratio}")

    windows = []
    total_length = len(data_sequence)

    # 计算时间轴调整后的参数
    adjusted_window_size = int(window_size * time_scale_ratio)
    adjusted_stride = int(stride * time_scale_ratio)

    # 确保调整后的窗口大小不超过总长度且至少为1
    adjusted_window_size = max(1, min(adjusted_window_size, total_length))
    adjusted_stride = max(1, adjusted_stride)

    print(f"调整后的窗口大小: {adjusted_window_size}, 步长: {adjusted_stride}")

    # 计算需要的总数据点数
    required_points = adjusted_window_size * window_size

    # 计算窗口数量
    num_windows = (total_length - required_points) // (adjusted_stride * window_size) + 1
    print(f"预计生成窗口数量: {num_windows}")

    for i in tqdm(range(num_windows), desc="生成窗口"):
        # 计算时间轴起始位置
        time_start = i * adjusted_stride * window_size
        time_end = time_start + required_points

        if time_end > total_length:
            break

        # 提取窗口数据
        window_data = data_sequence[time_start:time_end]

        # 重塑为图像 [time_scale_ratio * window_size, window_size]
        try:
            image = window_data.reshape(adjusted_window_size, window_size)

            # 如果时间轴被压缩，需要插值恢复到原始窗口大小
            # if time_scale_ratio != 1.0:
            #     from scipy import ndimage
            #     # 使用插值将图像调整到目标大小
            #     zoom_factors = (window_size / adjusted_window_size, 1.0)
...
def apply_realistic_mask(data, num_station_groups=3, min_stations_per_group=3, max_stations_per_group=10,
                         min_time_length=10, max_time_length=30):
    """
    应用更真实的掩膜模式：连续时间段 + 多站点同时缺失
    """
    ...
    for group_idx in range(num_station_groups):
        # 随机选择一组站点（3-10个站点）
        ...
        print(f"  掩膜组 {group_idx + 1}: 站点{station_indices} 在时间[{time_start}:{time_end}]缺失")

    return masked_data, mask


def apply_masks_to_datasets_realistic(train_data, val_data,
                                      num_station_groups=3, min_stations_per_group=3, max_stations_per_group=10,
                                      min_time_length=10, max_time_length=30):
    """
    对训练集和验证集应用更真实的掩膜处理
    """
    print("正在应用真实掩膜模式...")
    print(f"掩膜参数: {num_station_groups}个站点组, 每组{min_stations_per_group}-{max_stations_per_group}个站点")
    ...
    for i in tqdm(range(len(train_data)), desc="掩膜训练集"):
        ...
        masked_img, mask = apply_realistic_mask(original_img, num_station_groups,
    ...
    print(f"训练集掩膜比例: {np.mean(1 - train_masks):.4f}")
    print(f"验证集掩膜比例: {np.mean(1 - val_masks):.4f}")
...
def visualize_realistic_masks(train_data, masked_train_data, train_masks,
                              ...
                              completion_dataset, time_scale_ratio=1.0):
    """
    可视化真实掩膜模式
    """
    fig, axes = plt.subplots(3, 2, figsize=(16, 18))
    title_suffix = f" (时间轴尺度: {time_scale_ratio})" if time_scale_ratio != 1.0 else ""
    fig.suptitle(f'气象数据可视化 - 真实掩膜模式{title_suffix}', fontsize=16, fontweight='bold')
    ...
    if len(train_data) >= 1:
        ...
        axes[0, 0].set_title(f'训练集原始样本\n形状: {img_original.shape}{title_suffix}')
        ...
        axes[0, 1].set_title(f'训练集掩膜样本\n掩膜比例: {masked_ratio:.2%}{title_suffix}\n红色区域: 模拟缺失')
    ...
    if len(completion_dataset) >= 1:
        ...
        axes[2, 0].set_title(f'待补齐集样本\n原始NaN比例: {zero_ratio:.2%}{title_suffix}\n蓝色区域: 原始缺失')
        ...
        stats_text += f"掩膜比例: {np.mean(1 - train_masks):.2%}\n"
        if time_scale_ratio != 1.0:
            stats_text += f"时间轴尺度: {time_scale_ratio}"
...
def main():
    ...
    # 在这里设置时间轴尺度比例
    time_scale_ratio = 1.5# 1.0为原始尺度，>1.0时间轴压缩，<1.0时间轴拉伸
    ...
    # 使用时间轴尺度控制的滑动窗口生成
    if time_scale_ratio != 1.0:
        data_matrix = create_sliding_windows_with_time_scale(
            normalized_data,
            window_size=105,
            stride=105,
    ...
    visualize_realistic_masks(
        ...
        val_data, masked_val_data, val_masks,
        completion_dataset, time_scale_ratio
    )
