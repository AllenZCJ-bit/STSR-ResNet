...
def load_all_csv_data(csv_folder_path):
    ...
    for csv_file in tqdm(csv_files, desc="读取文件"):
        ...
        try:
            ...
            elif '最高温度(℃)' in df.columns:
                data = df['最高温度(℃)'].values#最高温度(℃)	最低温度(℃)	平均温度(℃)	平均水汽压(hPa)	平均相对湿度(%RH)	日降水	风速(最大)	平均风速(10m)	日照时数
            else:
...
def create_sliding_windows_fixed(data_sequence, window_size=105, stride=105):#25,105
    """使用固定间隔的滑动窗口生成图像"""
...
def visualize_realistic_masks(train_data, masked_train_data, train_masks,
    ...
    fig.suptitle('气象数据可视化 - 真实掩膜模式 (连续时间段+多站点缺失)', fontsize=16, fontweight='bold')
...
def main():
    ...
    data_matrix = create_sliding_windows_fixed(normalized_data, window_size=105, stride=105)#25,105
